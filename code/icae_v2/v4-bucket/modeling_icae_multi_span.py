...
class ICAE(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        self.icae = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch.float16 if training_args.bf16 is False else torch.bfloat16, attn_implementation="flash_attention_2", device_map="auto", resume_download=True)
        
        self.training = self.model_args.train    
        
        if self.training:    # indepedent model for gradient checkpointing
            self.decoder = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch.float16 if training_args.bf16 is False else torch.bfloat16, attn_implementation="flash_attention_2", device_map="auto", resume_download=True)
        ...
        self.memory_token_embed = nn.Embedding(self.q_mem_size + self.r_mem_size + 3, self.icae.config.hidden_size, padding_idx=None)
        
        self.vocab_size_with_mem = self.vocab_size + self.q_mem_size + self.r_mem_size # so, the mem tokens are in the range [self.vocab_size, self.vocab_size + self.mem_size)
        ...
        if self.training_args.use_r_tokens:
            self.append_sequence = torch.arange(self.vocab_size, self.vocab_size + self.q_mem_size + self.r_mem_size, dtype=torch.long, device=device).unsqueeze(0)
        else:
            self.append_sequence = torch.arange(self.vocab_size, self.vocab_size + self.q_mem_size, dtype=torch.long, device=device).unsqueeze(0)
    ...
    def forward(
        ...
        # Decoder part
        decoder_mem_flag = (prompt_answer_ids >= self.vocab_size) & (prompt_answer_ids < self.vocab_size + self.total_mem_size)  # [batch_size, prompt_length]
