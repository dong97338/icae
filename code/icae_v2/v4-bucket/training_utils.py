...
class LengthGroupedBucketSampler(Sampler):
    """
    (샘플의 길이 정보를 기반으로) 비슷한 길이의 샘플들을 하나의 배치로 묶는 Sampler.
    - ex) bucket_size=2048 => 최대 2048개 샘플을 모으는 건 아니고,
      "비슷한 길이 끼리" 일부 버킷으로 묶는 로직. 아래는 간단 예시:
       1) 전체 데이터를 길이에 따라 정렬
       2) 일정한 단위(그룹)마다 나누어서 batch size 단위로 샘플링
    """
    def __init__(
        self,
        data_source: Dataset,
        ...
        """
        Args:
            data_source: HF Dataset 혹은 custom Dataset
            batch_size: 1 mini-batch에 들어가는 sample 수
            lengths: 각 샘플의 길이를 담은 리스트 (ex: dataset[i]["length"])
            shuffle: 셔플 여부
            grouping_factor: 길이가 비슷한 샘플들끼리 묶을 때,
                             얼마나 세밀하게 그룹핑할 것인지 결정
                             (값이 작을수록 길이가 매우 비슷한 샘플끼리 묶임)
        """
        self.data_source = data_source
        self.batch_size = batch_size
        self.shuffle = shuffle
        self.lengths = lengths  # 예: [10, 512, 513, 100, 1024, ...]

        # (1) 데이터 인덱스를 길이 기준으로 정렬
        #     예: lengths_sorted = [(idx, length), ...] 길이 오름차순 정렬
        self.indices = sorted(range(len(self.lengths)), key=lambda i: self.lengths[i])

        # (2) 'grouping_factor' 개 단위로 잘라서 "비슷한 길이"끼리 그룹 만들기
        #     예: grouping_factor=50 -> 정렬된 목록을 50개씩 slice
        grouped = []
        for i in range(0, len(self.indices), grouping_factor):
            group = self.indices[i : i + grouping_factor]
        ...
        # (5) 이제 각 그룹을 이어붙여서 self.indices 재구성
        self.buckets = []
        for g in grouped:
            self.buckets.extend(g)

        # self.buckets에는 "길이가 비슷한 샘플"들이 연속으로 모여있음
        # => 나중에 batch_size 단위로 잘라서 __iter__에서 yield

    def __iter__(self):
        # 이터레이터: batch_size씩 끊어서 반환
        batch = []
...
class DataCollatorForBucketPadding:
    """
    배치 내 "최장 시퀀스" 길이에 맞춰 패딩만 하고, 잘라내지 않는다.
    (메모리 토큰도 포함하여 절대 잘리지 않음)
    """
    ...
    def __call__(self, examples):
        # examples는 [{"input_ids":..., "labels":..., "prompt_answer_ids":...}, ...]
        ...
        prompt_answer_ids = [torch.tensor(e["prompt_answer_ids"], dtype=torch.long) for e in examples]

        # 배치 내 최장 길이 찾기
        max_length_input = max(len(x) for x in input_ids)
        max_length_prompt = max(len(x) for x in prompt_answer_ids)
        max_length_labels = max(len(x) for x in labels)

        # 3가지 중 "가장 긴 것"으로 패딩 → (메모리 토큰이 잘리지 않도록 보장)
        # 만약 prompt가 가장 길다면 prompt 길이에 맞춰야 하고,
        # labels가 더 길면 그 길이에 맞춰야 함
        max_length = max(max_length_input, max_length_prompt, max_length_labels)

        if self.pad_to_multiple_of:
            max_length = ((max_length - 1) // self.pad_to_multiple_of + 1) * self.pad_to_multiple_of

        # 이제 max_length에 맞춰 패딩
        padded_input_ids = self._pad(input_ids, max_length, fill_value=self.pad_token_id)
        padded_prompt_answer_ids = self._pad(prompt_answer_ids, max_length, fill_value=self.pad_token_id)
        padded_labels = self._pad(labels, max_length, fill_value=-100)

        return {
            "input_ids": padded_input_ids,
            "prompt_answer_ids": padded_prompt_answer_ids,
            "labels": padded_labels
        }

    def _pad(self, sequences, max_length, fill_value=-100):
        batch_size = len(sequences)
        out = torch.full((batch_size, max_length), fill_value, dtype=torch.long)
...
def pretrain_tokenize_function(examples, model, mem, lm_ratio=0.0):
    ...
    label_output = model.tokenizer(examples["output"], truncation=False, padding=False, return_attention_mask=False)
