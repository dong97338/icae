...
class PipelineConfig:
    ...
    # 어느 스테이지를 실행할지 여부
    run_stages: List[bool] = field(
        default_factory=lambda: [True, True, True],
        metadata={"help": "Which stages to run (3-element boolean list)"}
    )
    # 각 스테이지별 epoch 수
    epochs: List[int] = field(
        default_factory=lambda: [1, 1, 1],
        metadata={"help": "Number of epochs for each stage (3-element list)"}
    )
    
    # 전체 체크포인트를 저장할 상위 폴더
    checkpoint_dir: str = field(
        default="/ssd1/donghyeon/icae/out_cc_ko",
        metadata={"help": "Folder to save stage-wise checkpoints. Each stage i -> stage_i subfolder"}
    )
    ...
    def get_stage_checkpoint_dir(self, stage_idx: int) -> str:
        """
        스테이지별로 체크포인트를 저장/로드할 하위 폴더 경로
        """
        return os.path.join(self.checkpoint_dir, f"stage_{stage_idx}")
