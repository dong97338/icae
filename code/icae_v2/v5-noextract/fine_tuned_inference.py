...
# Read the data file
file_path = "/ssd1/donghyeon/icae/code/icae_v2/cc_ko/stage3_train.jsonl"
lines = None
...
with torch.no_grad():
    with open("out_cc_ko_stage3_train.out", "w") as f:

        for line in tqdm(lines[:100]):
            # Tokenize input text
            data = json.loads(line)
            tokenized_input = model.tokenizer(data["input"], padding="max_length", max_length=512, truncation=True, return_attention_mask=False)
            tokenized_prompt = model.tokenizer(data["prompt"], padding=False, truncation=False, return_attention_mask=False)
