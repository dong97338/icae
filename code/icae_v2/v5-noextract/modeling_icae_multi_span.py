...
class ICAE(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        if self.tokenizer.pad_token_id is None:
            # eos_token_id가 있다면 이를 pad_token_id로 사용
    ...
    def forward(
        ...
        label_length: Optional[torch.LongTensor] = None,  # 추가 가능(실제로 사용 안함)
        **kwargs 
    ):
        # Encoder part
    ...
    def _compress2(
        ...
        for segment_idx in range(num_segments):
            ...
            compress_outputs[segment_idx*self.total_mem_size: self.total_mem_size*(segment_idx+1)] = segment_compress_outputs[mem_flag]
